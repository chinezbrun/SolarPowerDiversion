    ...
    try:
        ...
        if (time.time() - start_time) >10:                                        # try every 10 sec 
            ...
            if (time.time() - uptime_push_time) >60:                                 # try every 60 sec 
                ...
                if MQTT_active=='true':
                    publish.single('home-assistant/solar/power_diversion_uptime', uptime, hostname=MQTT_broker)        #uptime every minute
                    publish.single('home-assistant/solar/zamel/1', gpio_states[GPIO.input(17)], hostname=MQTT_broker) #divert AC load1
                    publish.single('home-assistant/solar/zamel/2', gpio_states[GPIO.input(18)], hostname=MQTT_broker) #divert AC load2
                    publish.single('home-assistant/solar/dc_divert', gpio_states[GPIO.input(22)], hostname=MQTT_broker) #divert DC
                    publish.single('home-assistant/solar/stop_UP', gpio_states[GPIO.input(23)], hostname=MQTT_broker) #stop U/P low SOC
